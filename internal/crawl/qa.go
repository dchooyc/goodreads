...
// RenderQAMarkdown produces crawl-quality-report.md.
func RenderQAMarkdown(r QAReport) string {
	var b strings.Builder
	b.WriteString("# Crawl Quality Report\n\n")
	if r.Passed {
	...
	fmt.Fprintf(&b, "| Raw rows | %d |\n", m.RawRows)
	fmt.Fprintf(&b, "| Canonical nonblank IDs | %d |\n", m.CanonicalNonblankIDs)
	fmt.Fprintf(&b, "| Blank ID rows | %d (%.2f%%) |\n", m.BlankIDRows, m.BlankIDRate*100)
	fmt.Fprintf(&b, "| Blank title rows | %d |\n", m.BlankTitleRows)
	fmt.Fprintf(&b, "| Null genre rows | %d |\n", m.NullGenreRows)
	fmt.Fprintf(&b, "| Duplicate extra rows | %d |\n", m.DuplicateExtraRows)
	fmt.Fprintf(&b, "| Same-title blank-ID rows | %d |\n", m.SameTitleBlankIDRows)
	fmt.Fprintf(&b, "| Previous canonical IDs | %d |\n", m.PreviousCanonicalIDs)
	fmt.Fprintf(&b, "| Old IDs missing from new | %d |\n", m.OldIDsMissingFromNew)
	fmt.Fprintf(&b, "| P0 missing | %d (undecided: %d) |\n", m.P0MissingCount, m.P0MissingUndecided)
	fmt.Fprintf(&b, "| P1 missing | %d |\n", m.P1MissingCount)
	fmt.Fprintf(&b, "| Previous top-1000 disappeared | %d |\n", m.Top1000Disappeared)

	if len(m.Top100MissingTitles) > 0 {
		b.WriteString("\n## Top missing titles by previous ratings\n\n")
		for _, t := range m.Top100MissingTitles {
			fmt.Fprintf(&b, "- %s\n", t)
		}
	}
